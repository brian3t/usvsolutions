...
function theme_shortcode_social_icons($atts, $content) {
	...
	foreach ( $atts as $att_key => $att ) {
		if ( $att_key != 'size' && $att_key != 'align' ) {
			$icons .= '<a href="' . $att . '" rel="nofollow" target="_blank"><img src="' . THEME_URI . '/images/icons/social/' . $att_key . '-' . $type .'.png" alt="' . ucfirst($att_key) . '" title="' . ucfirst($att_key) . '" /></a>';
		}
	}
	
	return <<<EOT
<div class="social-icons-box social-icons-box-$align">
$icons
</div>
EOT;
}
