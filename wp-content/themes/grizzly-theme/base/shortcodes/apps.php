...
function theme_shortcode_app($atts, $content = null, $code) {
	...
	foreach ( $apps as $app ) {
		...
		$list .= '<div class="title"><a href="' . $link . '">'.$title.'</a></div>';
...
<div itemscope itemtype="http://data-vocabulary.org/Review"><span itemprop="itemreviewed">{$app_title}</span><span itemprop="reviewer">{$fresh_source}</span><span itemprop="rating">{$score}</span>
</div>
</div>
RET;
}
